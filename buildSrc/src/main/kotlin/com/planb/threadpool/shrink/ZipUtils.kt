...
infix fun File.zipTo(path: String) {
    if (isFile) {
        File(path).zipOutputStream().zipFrom(Collections.singletonList(absolutePath))
    } else if (isDirectory) {
        File(path).zipOutputStream().zipFrom(listFiles()?.map { it.absolutePath }?.toList()?: emptyList())
    }
}
...
fun ZipOutputStream.zipFrom(srcs: List<String>) {
